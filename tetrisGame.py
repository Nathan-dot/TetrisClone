...
class TetrisGame(object):
    ...
    def die(self):
        ...
        istr = 'Game Over!'
        x = OFFSET_X + ((len(BLOCK) * WIDTH - len(istr)) >> 1)
        ...
        for y in range(HEIGHT):
            for x in range(WIDTH):
                if self.board[y][x] != EMPTY:
                    ...
                    self.gamewin.addstr(ty, tx, BLOCK, color)
    ...
    def draw_border(self):
        ...
        hrz_border = H_BORDER * (len(V_BORDER) * 2 + len(BLOCK) * WIDTH)
        self.gamewin.addstr(0, 0, hrz_border, color)
        self.gamewin.addstr(HEIGHT + 1, 0, hrz_border, color)

        for i in range(HEIGHT):
            self.gamewin.addstr(i + 1, 0, V_BORDER, color)
            self.gamewin.addstr(i + 1, len(BLOCK) * WIDTH + len(V_BORDER),
                                V_BORDER, color)
    ...
    def draw_info(self):
        ...
        for y, x in TETROMINOS[self.next_tetromino]:
            self.infowin.addstr(y + 1, len(BLOCK) * x + IOFF_X, BLOCK, color)
        ...
        values = [timestr, self.score, self.lines]
